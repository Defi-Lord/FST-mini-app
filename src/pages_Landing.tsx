...
export default function Landing({ onGetStarted, onRewards }: LandingProps) {
      ...
      <div className="container center hero" onClick={handleHeroClick}>
        ...
        <div className="copy-card">
          ...
          <h1 className="headline">
            Welcome to{' '}
            <span className="accent">Fantasy Sports Token (FST)</span> – the
            ultimate community for football fans who love to play, predict, and
            win.
          </h1>
          <p className="sub">
            Build your dream squad, enter weekly, monthly, and seasonal contests,
            and climb dynamic leaderboards. Connect your wallet to start — your
            journey begins now.
          </p>
...
function Style() {
  return (
    <style>{`
      ...
      .logo-shell {
        position: relative;
        width: min(56dvw, 460px);
        aspect-ratio: 1;
        display: grid;
        place-items: center;
        margin: 6dvh auto 18px;
        z-index: 2;
        transform: translateZ(0);
        filter: drop-shadow(0 40px 80px rgba(99,102,241,0.35))
          drop-shadow(0 18px 40px rgba(236,72,153,0.20));
      }
      ...
      .logo-mask {
        position: absolute;
        inset: 0;
        -webkit-mask-image: radial-gradient(60% 60% at 50% 50%, #000 60%, rgba(0,0,0,0) 100%);
        mask-image: radial-gradient(60% 60% at 50% 50%, #000 60%, rgba(0,0,0,0) 100%);
        pointer-events: none;
      }

      .copy-card {
        z-index: 3;
        width: min(92dvw, 880px);
        margin: 0 auto;
        padding: clamp(14px, 4.2dvw, 24px);
        border-radius: 18px;
      ...
      .brand {
        ...
        letter-spacing: .3px;
        opacity: .92;
        font-size: clamp(13px, 2.6dvw, 16px);
        margin-bottom: 6px;
      ...
      .headline {
        font-weight: 900;
        font-size: clamp(18px, 4.5dvw, 34px);
        line-height: 1.12;
        margin: 0 0 8px 0;
      }
      ...
      .sub {
        ...
        font-size: clamp(12px, 2.8dvw, 16px);
      }
