...
export default function AdminContests() {
  ...
  const [ready, setReady] = React.useState(false); // Wait for token/verification
  ...
  React.useEffect(() => {
    const init = async () => {
      try {
        ...
        // ensure token corresponds to admin
        await getMe(true);
        setReady(true);
      } catch (err: any) {
        console.warn('Admin verification failed:', err?.message || err);
        signOut();
      }
    };
    init();
