...
import { z } from "zod";
import { logEvent, ensureWallet } from "../utils/audit.js";
...
router.get("/nonce", async (req, res) => {
  try {
    ...
    return res.json({ wallet, nonce, message, expiresInSec: NONCE_TTL_SEC });
  } catch (e: any) {
...
router.post("/verify", express.json(), async (req, res) => {
  try {
    ...
    if (!nonceCookie) return res.status(401).json({ error: "missing nonce cookie" });
    ...
    const expectedMessage: string = issued.get(decoded.nonce) || decoded.msg || "";
    if (!expectedMessage) return res.status(400).json({ error: "nonce_message_missing" });
    ...
    const access = jwt.sign(
      { sub: wallet?.id || walletAddress, wallet: walletAddress, typ: "access" },
      JWT_SECRET,
      { expiresIn: "7d" }
    );
